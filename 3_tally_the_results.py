...
def main():
    ...
    while True:
        encrypted_vote_str = input(
            "Please provide encrypted vote as JSON (or press Enter to finish): "
        )
        ...
        encrypted_vote_alice = paillier.EncryptedNumber(
            public_key, int(encrypted_vote_data["alice"])
        )
        encrypted_vote_bob = paillier.EncryptedNumber(
            public_key, int(encrypted_vote_data["bob"])
        )
